...
import { $getSelection, $isRangeSelection, createCommand, COMMAND_PRIORITY_EDITOR, INSERT_LINE_BREAK_COMMAND, COMMAND_PRIORITY_HIGH, KEY_ENTER_COMMAND, COMMAND_PRIORITY_LOW, $createLineBreakNode, $createParagraphNode, $createTextNode, $createRangeSelection, $getNodeByKey, $createNodeSelection, $setSelection, SELECT_ALL_COMMAND, $getRoot, $isElementNode, } from 'lexical';
...
export const JupyterInputOutputPlugin = (props) => {
    ...
    useEffect(() => {
        return editor.registerMutationListener(JupyterInputNode, (mutatedNodes) => {
            ...
            for (const [nodeKey, mutation] of mutatedNodes) {
                if (mutation === 'destroyed') {
                    editor.update(() => {
                        let jupyterInputNodeUuid;
                        let jupyterOutputNodeUuid;
                        // Find the UUID for the destroyed input node
                        INPUT_UUID_TO_CODE_KEY.forEach((codeKey, codeUuid) => {
                            if (codeKey === nodeKey) {
                        ...
                        if (jupyterInputNodeUuid && jupyterOutputNodeUuid) {
                            ...
                            const outputNodeKey = OUTPUT_UUID_TO_OUTPUT_KEY.get(jupyterOutputNodeUuid);
                            if (outputNodeKey) {
    ...
    useEffect(() => {
        return editor.registerMutationListener(JupyterOutputNode, (mutatedNodes) => {
            // Skip if we're already moving nodes to prevent recursion
            if (isMovingNodes.current)
                return;
            for (const [nodeKey, mutation] of mutatedNodes) {
                if (mutation === 'updated') {
                    editor.update(() => {
                        ...
                        if (outputNode) {
                            const outputUuid = outputNode.getJupyterOutputNodeUuid();
    ...
    useEffect(() => {
        if (!editor.hasNodes([JupyterOutputNode])) {
            throw new Error('JupyterInputOutputPlugin: JupyterOutputNode not registered on editor');
        }
        if (!editor.hasNodes([JupyterInputNode])) {
            throw new Error('JupyterInputOutputPlugin: JupyterInputNode not registered on editor');
        }
    }, [editor]);
    useEffect(() => {
        return editor.registerCommand(INSERT_LINE_BREAK_COMMAND, _event => {
...
//# sourceMappingURL=JupyterInputOutputPlugin.js.map