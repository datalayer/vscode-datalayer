...
- **copy-external-deps.js** - Copies external npm dependencies (React, Lexical, etc.) from `node_modules/` to `dist/node_modules/` for VSIX packaging. Extensively filters out unnecessary files (tests, examples, source maps) to reduce bundle size.
- **copyZmqBinaries.js** - Copies ZeroMQ native module binaries and related dependencies (zeromq, cmake-ts, keytar, ws) to `dist/node_modules/` for VSIX packaging across all platforms.
