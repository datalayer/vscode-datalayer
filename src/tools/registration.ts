...
import { getActiveDocumentTool as getActiveDocumentLMTool } from "./operations/getActiveDocument";
import {
  startRuntimeOperation,
  connectRuntimeOperation,
} from "./operations/manageRuntime";
...
export function registerVSCodeTools(
  ...
  // Validate all definitions before registration (except VS Code-specific tools)
  const definitionsToValidate = definitions.filter(
    (def) => def.operation !== "getActiveDocument", // VS Code-specific, registered separately
  );
  const validation = validateToolDefinitions(definitionsToValidate, operations);
  if (!validation.valid) {
    const errorMessage = [
  ...
  // Register VS Code-specific tools that don't follow the standard pattern
  context.subscriptions.push(
    vscode.lm.registerTool(
      "datalayer_getActiveDocument",
      getActiveDocumentLMTool,
    ),
  );
  console.log(
    `[Datalayer Tools] ✓ Registered datalayer_getActiveDocument (VS Code-specific)`,
  );
}
