...
export function getActiveDatalayerNotebook(): vscode.Uri | undefined {
  // Datalayer notebooks are CUSTOM EDITORS, not text editors!
  // We MUST check tab groups, not activeTextEditor
  ...
  // Check if it's a Datalayer notebook by viewType or scheme
  const isDatalayerNotebookEditor =
    tabInput.viewType === "datalayer.jupyter-notebook" ||
    tabInput.uri.scheme === "datalayer";

  if (isDatalayerNotebookEditor && tabInput.uri.fsPath.endsWith(".ipynb")) {
    return tabInput.uri;
  }

  // Also check datalayer:// scheme (cloud notebooks)
  if (tabInput.uri.scheme === "datalayer") {
    return tabInput.uri;
  }
