...
import type { DatalayerSDK, Runtime, Environment } from "../../../core/lib/index.js";
...
export async function selectDatalayerRuntime(
    ...
    async (progress, token) => {
      ...
      try {
        ...
        if (runtimes.length > 0) {
          ...
          const isModel = firstRuntime && typeof firstRuntime.toJSON === 'function';
          console.log("[RuntimeSelector] First runtime is a model:", isModel);
          
          if (isModel) {
            const data = firstRuntime.toJSON();
            console.log("[RuntimeSelector] First runtime data from toJSON():", data);
          } else {
            console.log("[RuntimeSelector] First runtime raw object:", firstRuntime);
          }
        }

        // Get cached environments
        const environments = await EnvironmentCache.getInstance().getEnvironments(sdk);
        console.log("[RuntimeSelector] Found", environments.length, "environments");
    ...
    const getStartTime = (runtime: any) => {
      ...
      if (runtime && typeof runtime === 'object') {
        if ('startedAt' in runtime) {
          ...
          return runtimeData?.started_at ? new Date(runtimeData.started_at).getTime() : 0;
        } else {
          return runtime.started_at ? new Date(runtime.started_at).getTime() : 0;
        }
      }
      return 0;
    };
  ...
  if (validRuntimes.length > 0) {
    for (const runtime of validRuntimes) {
      ...
      if (runtime && typeof runtime === 'object') {
        ...
        if ('givenName' in runtime || 'podName' in runtime || 'uid' in runtime) {
          ...
          runtimeData = {
            ...
            uid: runtime.uid,
            environment_name: runtime.environmentName || runtime.environment_name,
            ...
            ingress: runtime.jupyterUrl || runtime.ingress || runtime.jupyter_url,
            token: runtime.jupyterToken || runtime.token || runtime.jupyter_token
          };
        } else if (typeof runtime.toJSON === 'function') {
      ...
      const displayName = runtimeData.given_name || 
                        runtimeData.pod_name || 
                        (runtimeData.uid ? `Runtime ${runtimeData.uid.slice(0, 8)}` : 'Unknown Runtime');
      ...
      const env = runtimeData.environment_title || runtimeData.environment_name || "unknown";
      ...
      if (runtimeData.started_at) {
        ...
        const timestamp = typeof runtimeData.started_at === 'number' ? 
        ...
        if (diffDays > 0) {
          ...
          timeInfo = ` • Started ${diffHours} hour${diffHours > 1 ? 's' : ''} ago`;
        } else if (diffMins > 0) {
          timeInfo = ` • Started ${diffMins} minute${diffMins > 1 ? 's' : ''} ago`;
        } else {
      ...
      const statusIcon = status === 'running' ? '$(play)' : status === 'stopped' ? '$(stop)' : '$(clock)';
  ...
  console.log("[RuntimeSelector] Adding create options for", environments.length, "environments");
  ...
  if (items.length === 0 || (items.length === 1 && items[0].kind === vscode.QuickPickItemKind.Separator)) {
    vscode.window.showInformationMessage("No runtimes or environments available");
    return undefined;
  }

  // Show QuickPick (progress is now dismissed)
  console.log("[RuntimeSelector] About to show QuickPick with", items.length, "items");
  console.log("[RuntimeSelector] QuickPick items:", items.map(item => ({
    label: item.label,
    action: item.action,
  ...
  console.log("[RuntimeSelector] User selected item:", selected ? {
  ...
  if (selected.runtime) {
    ...
    if ('uid' in runtime && typeof runtime.uid !== 'undefined') {
      ...
      ingress = (runtime as any).jupyterUrl || runtime.ingress || runtime.jupyter_url;
      token = (runtime as any).jupyterToken || runtime.token || runtime.jupyter_token;
    ...
    // Create new runtime
    console.log("[RuntimeSelector] User selected create action with environment:", {
...
async function createRuntime(
  ...
  const name = await vscode.window.showInputBox({
    ...
    validateInput: (value) => {
      console.log("[RuntimeSelector] Input validation called with value:", value);
  ...
  if (!name) {
    console.log("[RuntimeSelector] User cancelled input box or provided empty name");
    return undefined;
  }
  ...
  const creditsInput = await vscode.window.showInputBox({
    ...
    validateInput: (value) => {
      console.log("[RuntimeSelector] Credits validation called with value:", value);
      if (!value || value.trim().length === 0) {
  ...
  if (creditsInput === undefined) {
    ...
    console.log("[RuntimeSelector] User cancelled credits input, using default");
  ...
  return vscode.window.withProgress(
    ...
    async (progress) => {
      try {
        progress.report({ increment: 0, message: "Requesting runtime creation..." });

        // Create the runtime
        console.log("[RuntimeSelector] About to call SDK createRuntime with params:", {
          given_name: name,
        ...
        progress.report({ increment: 25, message: "Runtime created successfully!" });
        console.log("[RuntimeSelector] Created runtime object:", runtime);

        // Wait for the runtime to be ready with connection info
        progress.report({ increment: 25, message: "Waiting for runtime to be ready..." });
        ...
        while (attempts < maxAttempts) {
          ...
          if (readyRuntime && (readyRuntime.ingress || readyRuntime.jupyter_base_url)) {
            console.log("[RuntimeSelector] Runtime ready with connection info:", readyRuntime);
            break;
          }
          
          attempts++;
          progress.report({ increment: 2, message: `Waiting for runtime... (${attempts}s)` });
        }
        
        // Complete the progress to 100%
        progress.report({ increment: 100 - 50 - attempts * 2, message: "Complete!" });
        
        if (readyRuntime) {
          vscode.window.showInformationMessage(`Runtime "${name}" created and ready!`);
          return readyRuntime;
        }
        
        // If we still don't have connection info, return what we have
        console.log("[RuntimeSelector] Warning: Runtime created but may not have full connection info yet");
        vscode.window.showInformationMessage(`Runtime "${name}" created but still initializing`);
        return runtime;
