...
export class WebSocketKernelClient {
  ...
  constructor(
    ...
    console.log("[WebSocketKernelClient] Created with session:", this._sessionId);
    ...
    if (runtime && typeof runtime === 'object') {
      ...
      if ('uid' in runtime && typeof runtime.uid !== 'undefined') {
        console.log("[WebSocketKernelClient] Runtime is a model with properties");
        ...
        this._runtime = {
          ...
          environment_name: (runtime as any).environmentName || runtime.environment_name || '',
          // Try ingress first, then jupyterUrl as fallback
          ingress: runtime.ingress || (runtime as any).jupyterUrl || runtime.jupyter_url || '',
          // Try token first, then jupyterToken as fallback
          token: runtime.token || (runtime as any).jupyterToken || runtime.jupyter_token || '',
  ...
  public async connect(): Promise<void> {
    ...
    try {
      ...
      if (!ingressUrl || !token) {
        ...
        throw new Error(`Runtime missing connection info. Has ingress: ${!!ingressUrl}, Has token: ${!!token}`);
      }
  ...
  private async ensureKernel(): Promise<void> {
    try {
      ...
      if (listResponse.ok) {
        const kernels: any[] = await listResponse.json();
        if (kernels.length > 0) {
          this._kernelId = kernels[0].id;
          console.log("[WebSocketKernelClient] Using existing kernel:", this._kernelId);
          return;
        }
      }
      ...
      if (createResponse.ok) {
        ...
        throw new Error(`Failed to create kernel: ${createResponse.statusText}`);
      }
  ...
  private getWebSocketUrl(): string {
    const baseUrl = this._runtime.ingress!
      .replace(/^https?:/, "wss:")
      .replace(/\/$/, "");
  ...
  private onMessage(data: any): void {
    try {
      ...
      console.log("[WebSocketKernelClient] Received message:", msg.header.msg_type);
  ...
  public async execute(code: string): Promise<ExecutionResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const originalResolve = this._pendingRequests.get(msg.header.msg_id)!.resolve;
      ...
      try {
        this.sendMessage(msg);
        console.log("[WebSocketKernelClient] Sent execute request:", msg.header.msg_id);
      } catch (error) {
        clearTimeout(timeout);
        this._pendingRequests.delete(msg.header.msg_id);
        reject(error);
  ...
  public async interrupt(): Promise<void> {
    ...
    try {
      const baseUrl = this._runtime.ingress.replace(/\/$/, "");
      const response = await fetch(`${baseUrl}/api/kernels/${this._kernelId}/interrupt`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${this._runtime.token}`
        }
      ...
      console.error("[WebSocketKernelClient] Failed to interrupt kernel:", error);
    }
  }
  ...
  public async restart(): Promise<void> {
    ...
    try {
      const baseUrl = this._runtime.ingress.replace(/\/$/, "");
      const response = await fetch(`${baseUrl}/api/kernels/${this._kernelId}/restart`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${this._runtime.token}`
        }
