...
- **agentChatViewProvider.ts** - Webview view provider (`vscode.WebviewViewProvider`) for the `datalayerAgentChatView` view registered under the dedicated `datalayerChat` activity-bar container. Loads the compiled `dist/agentChat.js` bundle via `ui/templates/agentChatTemplate.ts` with a strict CSP + nonce. Forwards auth state (without the token), theme changes, and the runtime/agent list to the webview via `postMessage`, and wires up the typed `IAgentRuntimesClient` bridge plus the raw `fetch`/`WebSocket` network bridge so the embedded `<Chat>` UI from `@datalayer/agent-runtimes` can talk to the extension host without holding the auth token directly.
